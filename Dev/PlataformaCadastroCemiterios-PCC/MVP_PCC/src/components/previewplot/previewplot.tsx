...
import { getViewer, getFusionRoot } from "mapguide-react-layout/lib/api/runtime";
import { tr as xlate, tr } from "mapguide-react-layout/lib/api/i18n";
import { RuntimeMap } from "mapguide-react-layout/lib/api/contracts/runtime-map";
import {
    GenericEvent,
    IMapView,
...
import { MapCapturerContext, IMapCapturerContextCallback } from "mapguide-react-layout/lib/containers/map-capturer-context";
import { Slider, Button, Intent, Callout, HTMLSelect } from '@blueprintjs/core';
import { useActiveMapName, useActiveMapView, useActiveMapExternalBaseLayers, useViewerLocale, useAvailableMaps, usePrevious } from 'mapguide-react-layout/lib/containers/hooks';
import { setViewRotation, setViewRotationEnabled } from 'mapguide-react-layout/lib/actions/map';
import { debug } from 'mapguide-react-layout/lib/utils/logger';
import { useActiveMapState } from 'mapguide-react-layout/lib/containers/hooks-mapguide';
import { useReduxDispatch } from "mapguide-react-layout/lib/components/map-providers/context";

function getMargin() {
    /*
    var widget = getParent().Fusion.getWidgetsByType("PreviewPlot")[0];
    var margin;
...
export interface IPreviewPlotContainerOwnProps {

}

export interface IPreviewPlotContainerConnectedState {
...
export interface IPreviewPlotContainerDispatch {
    setViewRotation: (rotation: number) => void;
    setViewRotationEnabled: (enabled: boolean) => void;
}

export interface IPreviewPlotContainerState {
    title: string;
    subTitle: string;
    showLegend: boolean;
...
export const PreviewPlotContainer = () => {
    ...
    if (externalBaseLayers) {
        hasExternalBaseLayers = false;//externalBaseLayers.length > 0;
    }
    ...
    const url = `${getFusionRoot()}/widgets/PreviewPlot/PlotAsPDF.php`
    return <div className="component-quick-plot">
        <form id="Form1" name="Form1" target="_blank" method="post" action={url}>
            ...
            <div className="Title FixWidth">{xlate("QUICKPLOT_HEADER", locale)} style={{ display: 'none' }} </div>
            <label className="bp3-label" style={{ display: 'none' }} >
                {xlate("QUICKPLOT_TITLE", locale)}
                <input style={{ display: 'none' }} type="text" className="bp3-input bp3-fill" dir="auto" name="{field:title}" id="title" maxLength={100} value={title} onChange={onTitleChanged} />
            </label>
            <label className="bp3-label" style={{ display: 'none' }}>
                {xlate("QUICKPLOT_SUBTITLE", locale)}
                <input style={{ display: 'none' }} type="text" className="bp3-input bp3-fill" dir="auto" name="{field:sub_title}" id="subtitle" maxLength={100} value={subTitle} onChange={onSubTitleChanged} />
            </label>
                    ...
                    return <div>
                        ...
                        <label className="bp3-label" style={{ display: 'none' }}>
                            ...
                            {/*
                                ...
                                */}
                            <div className="bp3-select bp3-fill"  style={{ display: 'none' }} >
                        ...
                        <label className="bp3-label noselect"  style={{ display: 'none' }} >
            ...
            <div className="ButtonContainer FixWidth" style={{ display: 'none' }}>
