...
const Wpcc_TiposMovimento= ({obj_tiposmovimento_show}) => {
  const windowRef = useRef(null);  
  const menuRef = useRef(null);
  const tabRef = useRef(null);

  const [dataTiposMovimento, setDataTiposMovimento] = useState(null); 
  ...
  const handleClose = () => { 
    console.log('Close Tipos de Movimento');    
    var aux={ type: Actions.HIDE_GESTAOTIPOSMOVIMENTO, payload: "" };
    viewer.dispatch(aux);     
  };

  useEffect(() => {     
    const fetchdataTiposMovimento = async () => {//ok
      const url3 = apiEndpointCadastro + 'TipoMovimento';//ok 
      console.log(url3);
      ...
      .then(jsonData => {
        setDataTiposMovimento(jsonData); 
      })
      .catch(error => {
        console.error('There was a problem with your fetch operation:', error);
      });  
    } 
    fetchdataTiposMovimento();   
  }, [obj_tiposmovimento_show]);
  ...
  useEffect(() => {
        ...
        if (originalResponses.length==0){
          ...
          originalResponses.forEach((originalResponse) => {
            ...
            if (designacao === null || designacao === '') {
              transformedDescricao = `Tipo Movimento ${counter}`;
    ...
    if ((treeReferencia.current !=null )&&(dataTiposMovimento !=null )){
      const transformedResult = transformResponse(dataTiposMovimento); 
      treeReferencia.current.data.parse(transformedResult);
      setDataTiposMovimento(dataTiposMovimento);
    }
  }, [dataTiposMovimento]);

  useEffect(() => {
    if (obj_tiposmovimento_show !== false) {
      ...
      FormDadosEnt.current = new FormDHX(null,{ 
        ...
        rows: [
          {
            css: "pcc_input", type: "fieldset", label: "Tipo de Movimento: ", name: "g1", disabled: false, hidden: false,
            rows: 
            [ 
              { type: "spacer", height: 15 }, // Espaço antes das rows
              { css: "pcc_input", id: "objformname", type: "input", name: "nome", label: "Descrição:", placeholder: "Descrição" },
      ...
      function handleSave() {
        if (FormDadosEnt.current) {
          ...
          if(objformparent == "update"){
            ...
            onupdateWindow(objformid,objformnome); 
          }else{
            onsaveWindow(objformnome);
          }
  ...
  const ondeleteWindow = (objgeomid) => {
    const DeleteObjGeometry = (objgeomid)=> {
      try {				
        const savePretensaoObj = async () => {                   
          const url = apiEndpointCadastro + 'TipoMovimento';//ok   
          const jwtToken = authtoken;         
    ...
    dhx.confirm({
      ...
      text:"Tem a certeza que pretende remover o tipo de Movimento?",
      buttons:["sim", "nao"],
    ...
    const refreshDataTiposMovimento = async () => {//ok
      const url3 = apiEndpointCadastro + 'TipoMovimento';//ok 
      const jwtToken = authtoken; 
      fetch(url3, {
        headers: {
  ...
  const onsaveWindow = (objformnome) => {   
    const contSave = (objformnome)=> {   
        const saveTipoMovimento = async () => {  
          const url = apiEndpointCadastro + 'TipoMovimento/';//ok     
          const jwtToken = authtoken;    
          const response = await fetch(url, {
            method: 'POST',
  ...
  const onupdateWindow = (objformid,objformnome) => {   
    const contSave = (objformid,objformnome)=> {   
        const savePretensaoObj = async () => {                             
          const url = apiEndpointCadastro + 'TipoMovimento/';
          const jwtToken = authtoken;        
          const response = await fetch(url, {
            method: 'PUT',
            body: JSON.stringify({
...
Wpcc_TiposMovimento.propTypes = {
  obj_tiposmovimento_show: PropTypes.boolean,
};
export default Wpcc_TiposMovimento;
